...
export default function Home() {
  ...
  const [appliedDate, setAppliedDate] = useState<string>(getTodayISO()); 
        ...
        <div style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '16px', marginBottom: '30px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <input type="date" value={targetDate} onChange={(e) => setTargetDate(e.target.value)} style={{ padding: '8px', borderRadius: '6px', border: '1px solid #ccc', outline: 'none', color: '#000000', backgroundColor: '#ffffff' }} />
              <span style={{ color: '#555555' }}>까지</span>
              <button onClick={handleSearch} style={{ marginLeft: '10px', padding: '8px 16px', backgroundColor: '#0070f3', color: '#ffffff', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>적용하기</button>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            ...
            {filteredEvents.slice(0, visibleCount).map((event) => (
              <div key={event.id} style={{ border: '1px solid #eeeeee', borderRadius: '16px', padding: '20px', backgroundColor: '#ffffff', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 10px rgba(0,0,0,0.04)' }}>
                <div style={{ height: '180px', backgroundColor: '#f1f5f9', borderRadius: '12px', marginBottom: '15px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {event.image_url ? (
                    <img src={event.image_url.replace('http://', 'https://')} alt={event.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                  ) : (
                    <div style={{ fontSize: '3.5rem' }}>{event.category === '공연' ? '🎭' : '🎈'}</div>
                  )}
                </div>
                ...
                <a href={event.url} target="_blank" rel="noreferrer" style={{ display: 'block', marginTop: '20px', textAlign: 'center', backgroundColor: '#1a1a1a', color: '#ffffff', padding: '12px 0', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.95rem' }}>상세정보 / 예매하기</a>
              </div>
            ))}
          </div>
        )}
